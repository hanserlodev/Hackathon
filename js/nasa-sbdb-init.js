...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    console.log(
      `✅ Successfully loaded ${asteroids.length} asteroids from NASA SBDB`
    );
    ...
    asteroids.forEach((asteroid) => {
      ...
      console.log(
        `  ✓ ${asteroid.name}${asteroid.pha ? " ⚠️ PHA" : ""}${
          asteroid.neo ? " 🌍 NEO" : ""
        }`
      );
      console.log(
        `    → Diameter: ${asteroid.physical.diameter.toFixed(2)} km`
      );
      console.log(
        `    → Orbit: a=${asteroid.orbit.semiMajorAxis.toFixed(
          3
        )} AU, e=${asteroid.orbit.eccentricity.toFixed(3)}`
      );
    });
    ...
    if (meteorSelect) {
      ...
      asteroids.forEach((asteroid) => {
        ...
        const exists = Array.from(meteorSelect.options).some(
          (opt) => opt.value === asteroid.name
        );

        if (!exists) {
          const option = document.createElement("option");
          option.value = asteroid.name;
          option.textContent = `${asteroid.name} ${asteroid.pha ? "⚠️" : ""}${
            asteroid.neo ? "🌍" : ""
          }`;
          option.dataset.spkid = asteroid.spkId;
          option.dataset.diameter = (asteroid.physical.diameter * 1000).toFixed(
            0
          ); // km to m
          option.dataset.velocity = NASASBDB.estimateImpactVelocity(
            asteroid.orbit
          ).toFixed(1);
      ...
      console.log(
        `✅ Added ${asteroids.length} real asteroids to selector (total: ${meteorSelect.options.length})`
      );
    }
    ...
    if (meteorSelect) {
      meteorSelect.innerHTML =
        '<option value="manual">Custom Impact Parameters (NASA API Error)</option>';
    }
  }
});
...
function addOrbitViewerButton() {
  ...
  button.innerHTML =
    '<i class="fas fa-satellite"></i> View Orbital Data (NASA SBDB)';
...
async function showOrbitDataModal() {
  ...
  if (
    ...
    alert(
      "⚠️ Please select a real asteroid from the dropdown first.\n\nAvailable asteroids are marked with 🌍 (NEO) and ⚠️ (PHA)"
    );
    return;
  }
  ...
  try {
                ...
                <div class="orbit-modal-body">
                    ...
                    <div class="orbit-info-grid">
                        <div class="orbit-info-section">
                            ...
                            <ul>
                                <li><strong>Object Type:</strong> ${(
                                  data.type || "asteroid"
                                ).toUpperCase()}</li>
                                <li><strong>SPK-ID:</strong> ${
                                  data.spkId || "N/A"
                                }</li>
                                <li><strong>Near-Earth Object (NEO):</strong> ${
                                  data.neo ? "✅ Yes" : "❌ No"
                                }</li>
                                <li><strong>Potentially Hazardous (PHA):</strong> ${
                                  data.pha ? "⚠️ YES" : "✅ No"
                                }</li>
                                ${
                                  data.physical?.diameter &&
                                  data.physical.diameter > 0
                                    ? `<li><strong>Diameter:</strong> ${data.physical.diameter.toFixed(
                                        3
                                      )} km (${(
                                        data.physical.diameter * 1000
                                      ).toFixed(0)} m)</li>`
                                    : "<li><strong>Diameter:</strong> Not available</li>"
                                }
                                ${
                                  data.physical?.albedo &&
                                  data.physical.albedo > 0
                                    ? `<li><strong>Albedo:</strong> ${data.physical.albedo.toFixed(
                                        3
                                      )}</li>`
                                    : ""
                                }
                                ${
                                  data.physical?.magnitude &&
                                  data.physical.magnitude > 0
                                    ? `<li><strong>Absolute Magnitude (H):</strong> ${data.physical.magnitude.toFixed(
                                        2
                                      )}</li>`
                                    : ""
                                }
                        ...
                        <div class="orbit-info-section">
                            ...
                            <ul>
                                <li><strong>Semi-major Axis (a):</strong> ${(
                                  data.orbit.semiMajorAxis ||
                                  data.orbit.perihelionDistance ||
                                  0
                                ).toFixed(6)} AU</li>
                                <li><strong>Eccentricity (e):</strong> ${data.orbit.eccentricity.toFixed(
                                  6
                                )}</li>
                                <li><strong>Inclination (i):</strong> ${data.orbit.inclination.toFixed(
                                  4
                                )}°</li>
                                <li><strong>Longitude Ascending Node (Ω):</strong> ${data.orbit.ascendingNode.toFixed(
                                  4
                                )}°</li>
                                <li><strong>Argument Perihelion (ω):</strong> ${data.orbit.argPerihelion.toFixed(
                                  4
                                )}°</li>
                                ${
                                  data.orbit.meanAnomaly
                                    ? `<li><strong>Mean Anomaly (M):</strong> ${data.orbit.meanAnomaly.toFixed(
                                        4
                                      )}°</li>`
                                    : ""
                                }
                                <li><strong>Orbital Period:</strong> ${(
                                  data.orbit.period / 365.25
                                ).toFixed(
                                  2
                                )} years (${data.orbit.period.toFixed(
      1
    )} days)</li>
                            </ul>
                        </div>
                        ...
                        <div class="orbit-info-section">
                            ...
                            <ul>
                                <li><strong>Estimated Impact Velocity:</strong> ${impactVelocity.toFixed(
                                  2
                                )} km/s</li>
                                <li><strong>Relative to Earth Orbit:</strong> ${(
                                  impactVelocity - 29.78
                                ).toFixed(2)} km/s</li>
                                ${
                                  data.physical?.diameter
                                    ? `<li><strong>Kinetic Energy (at impact):</strong> ${(
                                        (0.5 *
                                          data.physical.diameter *
                                          1000 *
                                          1000 *
                                ...
                                ${
                                  data.physical?.diameter
                                    ? `<li><strong>Diameter for Sim:</strong> ${(
                                        data.physical.diameter * 1000
                                      ).toFixed(0)} m</li>`
                                    : ""
                                }
                                <li><strong>Velocity for Sim:</strong> ${impactVelocity.toFixed(
                                  1
                                )} km/s</li>
                            </ul>
                        </div>
                    ...
                    <div class="orbit-actions">
                        <button class="use-for-simulation" onclick="applyOrbitDataToSimulation('${selectedValue.replace(
                          /'/g,
                          "\\'"
                        )}')">
                        ...
                        <button class="view-sbdb" onclick="(function() {
                            const designation = '${selectedValue}'.match(/^\\d+/) ? '${selectedValue}'.match(/^\\d+/)[0] : '${
      data.spkId
        ? data.spkId.replace(/^2/, "")
    ...
    alert(
      `❌ Could not fetch orbital data for "${selectedValue}".\n\nError: ${error.message}\n\nPlease try:\n1. Check your internet connection\n2. Try another asteroid\n3. Check browser console for details`
    );
  }
}
...
window.applyOrbitDataToSimulation = async function (name) {
  try {
    ...
    alert(
      `✅ Simulation parameters updated with real NASA SBDB data!\n\n` +
        `Asteroid: ${data.name}\n` +
        `Diameter: ${
          data.physical?.diameter
            ? (data.physical.diameter * 1000).toFixed(0) + " m"
            : "Unknown"
        }\n` +
