...
class EarthMap2D {
  ...
  toggleSeismic(show) {
    ...
    this.circleLayers.seismic.bindTooltip(
      `Seismic Waves: ${radius.toFixed(
        1
      )} km (Magnitude ${this.currentEffects.earthquake.magnitude.toFixed(1)})`,
  ...
  toggleTsunami(show) {
    ...
    this.circleLayers.tsunami.bindTooltip(
      `Tsunami Propagation: ${radius.toFixed(
        1
      )} km (Height: ${this.currentEffects.tsunami.height.toFixed(1)}m)`,
      { permanent: false, direction: "top", className: "zone-tooltip" }
    );
  }
  ...
  toggleBlastWave(show) {
    ...
    this.circleLayers.blastWave.forEach((circle) =>
    ...
    const blastZones = [
      {
        radius: zones.glassBreakage,
        color: "#ffff00",
        fillColor: "#ffff99",
        opacity: 0.2,
        label: "1 psi - Glass Breakage",
      },
      {
        radius: zones.lightDestruction,
        color: "#ffd700",
        fillColor: "#ffeb99",
        opacity: 0.25,
        label: "2 psi - Light Damage",
      },
      {
        radius: zones.moderateDestruction,
        color: "#ffaa00",
        fillColor: "#ffcc66",
        opacity: 0.3,
      ...
      {
        radius: zones.severeDestruction,
        color: "#ff6600",
        fillColor: "#ff9933",
        opacity: 0.4,
        label: "10 psi - Severe Damage",
      },
      {
        radius: zones.totalDestruction,
        color: "#ff0000",
        fillColor: "#ff4444",
        opacity: 0.5,
        label: "20 psi - Total Destruction",
      },
    ];
    ...
    blastZones.forEach((zone, index) => {
      ...
      circle.bindTooltip(`${zone.label}: ${zone.radius.toFixed(1)} km`, {
        permanent: false,
        direction: "top",
