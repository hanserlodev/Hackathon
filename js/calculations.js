class ImpactCalculations {
  ...
  calculateAtmosphericEntry(diameter, velocity, density, angle) {
    ...
    const airDensity =
      this.AIR_DENSITY_SEA_LEVEL *
      Math.exp(-impactAltitude / this.SCALE_HEIGHT);
    ...
    const dragParameter =
      (dragCoeff * airDensity * H) / (2 * ballisticCoeff * sinAngle);
  ...
  calculateKineticEnergy(mass, velocity, diameter, density, angle) {
    ...
    const entry = this.calculateAtmosphericEntry(
      diameter,
      velocity,
      density,
      angle
  ...
  calculateCraterDiameter(diameter, velocity, density, angle, energyJoules) {
    ...
    const pi2 =
      (mass / this.TARGET_DENSITY) *
      Math.pow(energyPerMass / (this.GRAVITY * projectileLength), gScaling.mu);
    ...
    const craterDiameter =
      2 * Math.pow((3 * craterVolume) / (Math.PI * 0.2), 1 / 3);
  ...
  estimateCasualtiesWithInfrastructure(
    ...
    const severeArea =
      Math.PI *
      (Math.pow(zones.severeDestruction, 2) -
        Math.pow(zones.totalDestruction, 2));
    const moderateArea =
      Math.PI *
      (Math.pow(zones.moderateDestruction, 2) -
        Math.pow(zones.severeDestruction, 2));
    const lightArea =
      Math.PI *
      (Math.pow(zones.lightDestruction, 2) -
        Math.pow(zones.moderateDestruction, 2));
    ...
    const moderateFatalities = moderateArea * populationDensity * 0.3;
    const lightFatalities = lightArea * populationDensity * 0.05;

    const totalFatalityCount =
      totalFatalities + severeFatalities + moderateFatalities + lightFatalities;
  ...
  estimateCasualties(energyJoules, populationDensity, energyMegatons = null) {
    const megatons = this.normalizeMegatons(energyJoules, energyMegatons);
    const totalDestructionRadius = this.calculateTotalDestructionZone(
      energyJoules,
      megatons
    );
    const severeDestructionRadius = this.calculateSevereDestructionZone(
      energyJoules,
      megatons
    );
    const moderateDestructionRadius = this.calculateModerateDestructionZone(
      energyJoules,
      megatons
    );
    ...
    const severeDestructionArea =
      Math.PI * Math.pow(severeDestructionRadius, 2);
    const moderateDestructionArea =
      Math.PI * Math.pow(moderateDestructionRadius, 2);

    const fatalitiesTotal = totalDestructionArea * populationDensity * 0.95;
    const fatalitiesSevere =
      (severeDestructionArea - totalDestructionArea) * populationDensity * 0.5;
    const fatalitiesModerate =
      (moderateDestructionArea - severeDestructionArea) *
      populationDensity *
      0.1;

    const totalFatalities =
      fatalitiesTotal + fatalitiesSevere + fatalitiesModerate;
  ...
  calculateTsunamiHeight(
    ...
    const amplitude =
      Math.pow(tsunamiEnergy / (rho * g * Math.pow(waterDepth, 2)), 0.25) *
      waterDepth;
  ...
  calculateSecondaryEffectTiming(
    energyJoules,
    distance,
    energyMegatons = null
  ) {
  ...
  calculateAllEffects(
    ...
    const materialDensity =
      this.densityValues[densityKey] ?? this.densityValues.stone;
    ...
    const atmosphericEntry = this.calculateAtmosphericEntry(
      diameter,
      velocity,
      materialDensity,
      angle
    ...
    const totalDestructionZone = this.calculateTotalDestructionZone(
      energy,
      energyMegatons
    );
    const earthquakeMagnitude = this.calculateEarthquakeMagnitude(
      energy,
      energyMegatons
    );
    ...
    const tsunamiRadius = this.calculateTsunamiRadius(
      energy,
      energyMegatons,
      isOceanImpact
    );
    const fireRadius = this.calculateFireRadius(energy, energyMegatons);
    const dustCloudRadius = this.calculateDustCloudRadius(
      energy,
      energyMegatons
    );
    ...
    const casualties = this.estimateCasualtiesWithInfrastructure(
      energy,
      populationDensity,
  ...
  classifyImpact(energyMegatons) {
    if (energyMegatons < 0.001) {
      ...
      return {
        level: "Regional",
        description: "Regional impact, significant damage.",
      };
    } else if (energyMegatons < 10) {
      return {
        level: "Continental",
        description: "Continental impact, massive devastation.",
      };
