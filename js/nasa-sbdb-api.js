...
export async function getAsteroidData(designation) {
  try {
    ...
    const url = `${NASA_SBDB_API}?sstr=${encodeURIComponent(
      designation
    )}&full-prec=true`;
    ...
    const apiData = {
      ...
      orbit: {
        ...
        eccentricity: parseFloat(
          data.orbit?.elements?.find((e) => e.name === "e")?.value || 0
        ),
        semiMajorAxis: parseFloat(
          data.orbit?.elements?.find((e) => e.name === "a")?.value || 0
        ), // AU
        inclination: parseFloat(
          data.orbit?.elements?.find((e) => e.name === "i")?.value || 0
        ), // degrees
        ascendingNode: parseFloat(
          data.orbit?.elements?.find((e) => e.name === "om")?.value || 0
        ), // degrees
        argPerihelion: parseFloat(
          data.orbit?.elements?.find((e) => e.name === "w")?.value || 0
        ), // degrees
        meanAnomaly: parseFloat(
          data.orbit?.elements?.find((e) => e.name === "ma")?.value || 0
        ), // degrees
        period: parseFloat(
          data.orbit?.elements?.find((e) => e.name === "per")?.value || 0
        ), // days
      },
  ...
  try {
    ...
    if (filters.diameterMin !== undefined) {
      conditions.push({
        field: "diameter",
        op: "GT",
        value: filters.diameterMin.toString(),
      });
    }
    ...
    if (filters.diameterMax !== undefined) {
      conditions.push({
        field: "diameter",
        op: "LT",
        value: filters.diameterMax.toString(),
      });
    }
...
export async function getNotableAsteroids() {
  ...
  for (const designation of notableNames) {
    try {
      ...
      if (knownData) {
        ...
        failCount++;
        console.warn(
          `❌ Could not fetch data for ${designation}:`,
          error.message
        );
      }
    }
  ...
  console.log(
    `✅ Loaded ${successCount}/${notableNames.length} asteroids (${failCount} failed)`
  );
...
export async function updateFamousMeteoriteSelector() {
  ...
  try {
    ...
    asteroids.forEach((asteroid) => {
      ...
      option.dataset.velocity = estimateImpactVelocity(asteroid.orbit).toFixed(
        1
      );
