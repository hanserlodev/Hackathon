...
function classifyShockWave(energyMegatons, totalDestructionZone) {
  ...
  const overpressure = calculateOverpressure(
    energyMegatons,
...
function classifySeismic(magnitude) {
  ...
  for (const level of SEISMIC_LEVELS) {
    ...
    const rangeMatch = level.magnitudeRange.match(
      /(\d+\.\d+)\s*-\s*(\d+\.\d+)/
    );
    if (rangeMatch) {
      ...
      // Handle "3.0 - 4.9" case
      const parts = level.magnitudeRange
        .split("-")
...
function generatePriorityActions(shockWave, seismic, thermal, infrastructure) {
  ...
  if (infrastructure.critical.hospitals > 0) {
    actions.add(
      `Secure ${infrastructure.critical.hospitals} hospital(s) in impact zone`
    );
  }
  if (infrastructure.critical.fireStations > 0) {
    actions.add(
      `Coordinate with ${infrastructure.critical.fireStations} fire station(s)`
    );
  }
  if (infrastructure.critical.police > 0) {
    actions.add(
      `Deploy emergency services from ${infrastructure.critical.police} police station(s)`
    );
  }
...
export function classifySimulationEffects(effects, overpassData) {
  ...
  const triageLevel =
    TRIAGE_LEVELS.find((t) => t.level === maxLevel) || TRIAGE_LEVELS[0];
...
export function generateSeverityReport(classification) {
  ...
  return `
        <div class="severity-report">
            <div class="overall-triage" style="border-color: ${
              overallSeverity.color
            }; background: ${overallSeverity.color}22;">
                <div class="triage-badge" style="background: ${
                  overallSeverity.color
                };">
                ...
                <div class="triage-info">
                    ...
                    <p class="attention-time">Attention Required: ${
                      overallSeverity.attentionTime
                    }</p>
                </div>
            </div>
            ...
            <div class="severity-categories">
                <div class="severity-section shock-wave-section">
                    ...
                    <div class="severity-details">
                        ...
                        <div class="impacts-grid">
                            ${Object.entries(shockWave.impacts)
                              .map(
                                ([sector, impact]) => `
                ...
                <div class="severity-section seismic-section">
                    ...
                    <div class="severity-details">
                        ...
                        <div class="impacts-grid">
                            ${Object.entries(seismic.impacts)
                              .map(
                                ([sector, impact]) => `
                                <div class="impact-item">
                                    <strong>${sector}:</strong> ${impact}
                ...
                <div class="severity-section thermal-section">
                    ...
                    <div class="severity-details">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="value">${
                              thermal.actualRadius
                            } km</span>
                        </div>
                        <div class="detail-row">
                            <span class="label">Thermal Diameter:</span>
                            <span class="value">${
                              thermal.actualDiameter
                            } km</span>
                        </div>
                        <div class="detail-row">
                            <span class="label">Temperature:</span>
                            <span class="value">${
                              thermal.temperatureRange
                            }</span>
                        </div>
                        <div class="impacts-grid">
                            ${Object.entries(thermal.impacts)
                              .map(
                                ([sector, impact]) => `
                                <div class="impact-item">
                                    <strong>${sector}:</strong> ${impact}
            ...
            <div class="infrastructure-analysis">
                ...
                <div class="infrastructure-stats">
                    <div class="stat-card">
                        <div class="stat-value">${
                          infrastructure.affectedPopulation
                        }</div>
                    ...
                    <div class="stat-card">
                        <div class="stat-value">${
                          infrastructure.criticalFacilitiesCount
                        }</div>
                ...
                <div class="critical-facilities">
                    ...
                    <ul>
                        ${
                          infrastructure.critical.hospitals > 0
                            ? `<li><i class="fas fa-hospital"></i> Hospitals: ${infrastructure.critical.hospitals}</li>`
                            : ""
                        }
                        ${
                          infrastructure.critical.fireStations > 0
                            ? `<li><i class="fas fa-fire-extinguisher"></i> Fire Stations: ${infrastructure.critical.fireStations}</li>`
                            : ""
                        }
                        ${
                          infrastructure.critical.police > 0
                            ? `<li><i class="fas fa-shield-halved"></i> Police Stations: ${infrastructure.critical.police}</li>`
                            : ""
                        }
                        ${
                          infrastructure.critical.schools > 0
                            ? `<li><i class="fas fa-school"></i> Schools: ${infrastructure.critical.schools}</li>`
                            : ""
                        }
                        ${
                          infrastructure.critical.fuelStations > 0
                            ? `<li><i class="fas fa-gas-pump"></i> Fuel Stations: ${infrastructure.critical.fuelStations}</li>`
                            : ""
                        }
                        ${
                          infrastructure.criticalFacilitiesCount === 0
                            ? "<li>No critical facilities detected in impact zone</li>"
                            : ""
                        }
                    </ul>
                </div>
