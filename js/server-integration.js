...
class ServerIntegration {
    ...
    try {
      ...
      if (result.status === "success") {
        ...
        this.showNotification(
          "Simulación 2D iniciada correctamente",
          "success"
        );
      ...
      this.showNotification(
        `Error al iniciar simulación 2D: ${error.message}`,
        "error"
      );
      return false;
    }
  ...
  async stopSimulation2D() {
    try {
      ...
      this.showNotification(
        `Error al detener simulación 2D: ${error.message}`,
        "error"
      );
      return false;
    }
  ...
  async init() {
    ...
    if (isConnected) {
      ...
      this.showNotification(
        "Python server is not available. 2D simulation will not be available.",
        "warning"
      );
    }
...
if (typeof MeteorSimulation !== "undefined") {
  class MeteorSimulationWith2D extends MeteorSimulation {
    ...
    async start2DSimulation() {
      ...
      // Start the 2D simulation.
      const success = await this.serverIntegration.startSimulation2D(
        simulationData
      );
